...
<c:if test="${not empty winDetail}">
                ...
                <td width="20%">
                    <c:if test="${noReadNum>0}">
                        <input class="btn btn-xs btn-success" value="全部已阅" style="width:100%;"
                               onclick="updateAllWinnerReadStatus('${userId}')">
                    </c:if>
                    <c:if test="${noReadNum==0}">
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-striped mb-none" style="font-size: 10px;">
            ...
            <tbody>
            <c:forEach var="dt" items="${winDetail}">
...
<script>
    function reloadWin() {
        $('#dlgWin').dialog('destroy');
        $("#load_content").load("win/gameRecord_winner?startDate=" + $('#startDate').val() + "&endDate=" + $('#endDate').val());
    }

    <%--更新所有状态为已读 大赢家--%>
    function updateAllWinnerReadStatus(userId) {
        var startDate = $('#startDate').val();
        var endDate = $('#endDate').val();
        $.post("win/gameRecord_winner_updateAllStatus", {
            userId: userId,
            startDate: startDate,
            endDate: endDate
        }, function () {
