...
<c:if test="${not empty hostDetail}">
    <div>
                ...
                <td width="20%">
                    <c:if test="${noReadNum>0}">
                        <input class="btn btn-xs btn-success" value="全部已阅" style="width:100%;"
                               onclick="updateAllHostReadStatus('${userId}')">
                    </c:if>
                    <c:if test="${noReadNum==0}">
    ...
    <div class="table-responsive scroll">
        <table class="table table-bordered table-striped mb-none" style="font-size: 10px;">
            ...
            <tbody>
            <c:forEach var="dt" items="${hostDetail}">
...
<div class="dialog-button">
    <a href="javascript:layer.closeAll();" class="btn btn-default btn-block"
       style="height:35px">关闭</a>
</div>

<script>
    function reloadHost() {
        $('#dlgHost').dialog('destroy');
        $("#load_content").load("win/gameRecord_host?startDate=" + $('#startDate').val() + "&endDate=" + $('#endDate').val());
    }

    <%--更新所有状态为已读 大赢家--%>
    function updateAllHostReadStatus(userId) {
        var startDate = $('#startDate').val();
        var endDate = $('#endDate').val();
        $.post("win/gameRecord_owner_updateAllStatus", {
            userId: userId,
            startDate: startDate,
            endDate: endDate
        }, function () {
